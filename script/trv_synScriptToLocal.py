...
def trv_synScrpt2Loc():
	...
	lsAllSrcScrpt=[item for item in lsTrvScrptDir if item.endswith(‘.mel’)]
	lsAllSrcScrpt+=[item for item in lsTrvScrptDir if item.endswith(‘.py’)]

	usrScrptDir=pmc.internalVar(usd=1)
	lsDstScrptDir=os.listdir(usrScrptDir)
	lsAllDstScrpt=[item for item in lsDstScrptDir if item.endswith(‘.mel’)]
	lsAllDstScrpt+=[item for item in lsDstScrptDir if item.endswith(‘.py’)]
