...
def trv_pin2Sim():
    ...
    pinSel[0].worldInverseMatrix >>dcmpMtrx.inputMatrix
    if slState=='GEO':
        pinSel[0].scale >> multScl.input1
    elif slState=='CPN':
        multScl.input.set([1,1,1])
    dcmpMtrx.outputTranslate >> multScl.input2
    multScl.output >> inverseTransLoc[0].translate
    dcmpMtrx.outputRotate >> inverseTransLoc[0].rotate
    pmc.refresh(su=1)
    pmc.bakeResults(inverseTransLoc[0],sm=1,t=(pmc.playbackOptions(q=1,min=1), pmc.playbackOptions(q=1,max=1)),sb=1,at=['tx','ty','tz','rx','ry','rz'])
    pmc.refresh(su=0)
    multScl.output // inverseTransLoc[0].translate
    dcmpMtrx.outputRotate // inverseTransLoc[0].rotate
...
def createRootCurve():
    invertRootCurve=pmc.circle(n='invertTransRoot',nr=(0,1,0))
